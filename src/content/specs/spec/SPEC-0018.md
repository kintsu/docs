...
updates:
  - author: joshua-auchincloss
    date: 2025-12-27
    description: Aligned with workspace manifests and fixed inconsistent keys
  - author: joshua-auchincloss
    date: 2025-12-25
...
- Consistent dependency resolution behaviour
...
- `authors` - Array of author objects with `name` (required) and `email` (optional) fields. Authors with email receive registry attributions with public profile tracking; authors without email are displayed by name only.
...
- `license-text` - Full license text or expanded identifier (optional)
...
Fields supporting file-based content (`description`, `license-text`, `readme`) accept either inline strings or file paths. The parser deserializes these as a tagged union:
...
During compilation, `Path` variants are resolved relative to the manifest location and read into memory.
