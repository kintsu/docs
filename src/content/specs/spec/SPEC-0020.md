...
updates:
  - author: joshua-auchincloss
    date: 2025-12-31
    description: Added form metadata extraction requirements for form-required types
  - author: joshua-auchincloss
    date: 2025-12-25
...
**Form metadata extraction**: For fields with form attributes, extract form metadata and include in field definition. Form-required types (`decimal`, `duration`, `f8`, `complex`) MUST have form metadata present; missing form on these types is a bundle generation error. The form metadata structure varies by type:

- `decimal`: `{ "precision": P, "scale": S }` where P is 1-34 and S is 0-P
- `duration`: `{ "variant": V }` where V is one of `ms`, `us`, `ns`, `iso8601`, `iso8601ext`
- `f8`: `{ "chain": [{ "format": "e4m3" }] }` or `{ "chain": [{ "format": "e5m2" }] }`
- `complex`: `{ "chain": [{ "format": F }], "precision": P }` where F is format and P is float precision
...
- Form-required types (`decimal`, `duration`, `f8`, `complex`) must have form metadata present
- Form metadata must match [TSY-0015](/specs/tsy/tsy-0015) rules (precision limits, valid variants)
...
- Form metadata on form-required types must be present and valid
