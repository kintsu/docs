...
This SPEC documents the compiler's deterministic behaviour for parsing, validating, and registering enum types. It specifies the value type inference algorithm (fork parsing to determine integer vs. string), discriminant assignment rules, variant name uniqueness validation, and registration into the type registry. This SPEC is normative for implementers who must reproduce the compiler's enum handling.
