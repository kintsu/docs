...
This SPEC documents the compiler's deterministic behaviour for extracting and registering anonymous structs. It specifies the exact extraction algorithm, the name-generation context-tracking mechanism, and the order of operations that ensure generated structs are registered before their containing types. This SPEC is normative for implementers who must reproduce the compiler's anonymous-struct handling.
...
After extraction, generated structs are registered into the `TypeRegistry` during the normal struct-registration workflow ([SPEC-0002](/specs/spec/spec-0002)).
