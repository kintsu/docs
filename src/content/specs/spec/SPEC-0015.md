...
_Figure 1: Global Type Registry structure. The registry maintains a thread-safe BTreeMap (wrapped in Arc<Mutex<...>>) mapping NamedItemContext keys to FromNamedSource<Spanned<ResolvedType>> values. Uses BTreeMap for deterministic iteration order._

**Internal Structure** (from `parser/src/ctx/registry.rs`):

```rust
pub struct TypeRegistry {
    inner: Arc<Mutex<BTreeMap<NamedItemContext, FromNamedSource<Spanned<ResolvedType>>>>>,
}
```

Note: Uses `std::sync::Mutex` (not `tokio::sync::Mutex`) since operations are synchronous and short-lived.
...
pub fn register(
    ...
    self.with_lock_mut(|inner| {
        ...
        if inner.contains_key(&path) {
            return Err(Error::DuplicateType { name: path.display() }
                .with_span(span.start, span.end));
        }

        inner.insert(
            path.clone(),
            ResolvedType { kind, qualified_path: path }
...
Note: Values are wrapped in `FromNamedSource<Spanned<...>>` for source tracking.
...
- [x] [SPEC-0015](/specs/spec/spec-0015).AC-8: Lock acquisition handles poisoning with error return
- [x] [SPEC-0015](/specs/spec/spec-0015).AC-9: Introspection provides list of all registered types
- [x] [SPEC-0015](/specs/spec/spec-0015).AC-10: Candidate ordering is deterministic (import order + local last)

## References

- [AD-0002](/specs/ad/ad-0002) - Parallel Compilation Architecture
- [RFC-0014](/specs/rfc/rfc-0014) - Parallel Compilation Design
- [RFC-0015](/specs/rfc/rfc-0015) - Type Registry Design
