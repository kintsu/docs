...
This SPEC documents the compiler's deterministic behaviour for parsing, validating, and applying variant tagging attributes. It specifies attribute parsing, validation phases, inheritance resolution, name normalization, and metadata propagation to code generators. This SPEC is normative for implementers who must reproduce the compiler's tagging behaviour.
...
| Error Variant  | Tagging Behaviour                                        |
| -------------- | -------------------------------------------------------- |
...
The compiler emits specific error messages for tagging issues. See [ERR-0009](/specs/err/err-0009) for complete error definitions with examples.

| Code    | Message                                                           | Phase |
| ------- | ----------------------------------------------------------------- | ----- |
| KTG2001 | `attribute 'tag' parameter 'name' must be a string literal`       | 2     |
| KTG3001 | `attribute 'tag' specifies multiple tagging styles`               | 2     |
| KTG2002 | `attribute 'tag' can only be applied to oneof or error types`     | 2     |
| KTG3002 | `internal tag field '{name}' conflicts with variant field`        | 5     |
| KTG3003 | `adjacent tag field and content field must have different names`  | 5     |
| KTG3004 | `untagged oneof contains duplicate variant types`                 | 5     |
| KTG3005 | `untagged oneof contains structurally indistinguishable variants` | 5     |
| KTG2003 | `internal tagging requires struct content, found {kind}`          | 5     |
