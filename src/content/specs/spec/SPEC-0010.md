...
This SPEC documents the compiler's deterministic behaviour for loading, organizing, and compiling namespace definitions. It specifies the namespace loading process (file discovery and parallel reading), file resolution behaviour (merging multiple files into same namespace), dependency graph construction (import-based relationships), compilation order (breadth-first by depth with parallelism), and the eight type resolution stages executed for each namespace. This SPEC is normative for implementers who must reproduce the compiler's namespace handling behaviour.
