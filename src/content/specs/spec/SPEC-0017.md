...
> [!NOTE]
> For complete error definitions with examples and help text, see [ERR-0010](/specs/err/err-0010) (Type Expression Errors).

### Syntax Errors

| Code    | Message                                     | Cause            |
| ------- | ------------------------------------------- | ---------------- |
| KTE0001 | `expected '[' after operator name`          | Missing bracket  |
| KTE0002 | `expected ']' to close operator`            | Unclosed bracket |
| KTE0003 | `expected identifier in selector list`      | Invalid selector |
| KTE0004 | `expected ',' between target and selectors` | Missing comma    |

### Type Errors

| Code    | Message                              | Cause                      |
| ------- | ------------------------------------ | -------------------------- |
| KTE2001 | `expected struct type, found {type}` | Wrong target for struct op |
| KTE2002 | `expected oneof type, found {type}`  | Wrong target for oneof op  |
| KTE2003 | `expected array type, found {type}`  | Wrong target for ArrayItem |
| KTE2004 | `cannot access fields on {type}`     | Field access on scalar     |

### Resolution Errors

| Code    | Message                                           | Cause                  |
| ------- | ------------------------------------------------- | ---------------------- |
| KTE1001 | `field '{name}' not found in struct '{struct}'`   | Unknown field          |
| KTE1002 | `variant '{name}' not found in oneof '{oneof}'`   | Unknown variant        |
| KTE4001 | `empty selector list not allowed`                 | Empty Pick/Extract     |
| KTE4002 | `no fields remain after omitting all fields`      | Omit leaves nothing    |
| KTE4003 | `no variants remain after excluding all variants` | Exclude leaves nothing |
| KTE5001 | `cyclic type expression detected`                 | Self-referential expr  |

### Warnings

| Code    | Message                                                     | Cause                  |
| ------- | ----------------------------------------------------------- | ---------------------- |
| KTE8001 | `duplicate selector '{name}' ignored`                       | Repeated field/variant |
| KTE8002 | `Partial has no effect on already-optional field '{name}'`  | Redundant Partial      |
| KTE8003 | `Required has no effect on already-required field '{name}'` | Redundant Required     |
