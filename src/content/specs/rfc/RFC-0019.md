...
keywords = ["api", "types"]
license = "MIT"
license_text = "Full license text here"
readme = "# My Package\n\nDocumentation here"
```
...
The `description`, `license_text`, and `readme` fields support file-based content using the `{ path = "..." }` syntax:
...
| `package.license_text`    | String or `{ path: String }` | No       |
...
The parser deserializes these as a tagged union:
...
- [ ] AC-9: Size limits are enforced for file-based fields
- [ ] AC-10: Clear errors for missing or unreadable files (KPK4003, KPK4004)
