...
updates:
  - author: joshua-auchincloss
    date: 2025-12-27
    description: Moved implementation details to SPEC-0018, size limits to registry spec
  - author: joshua-auchincloss
    date: 2025-12-26
    description: Added file-based fields (description, license, readme) with path syntax
  - author: joshua-auchincloss
    date: 2025-12-25
...
The `email` field is optional. Authors with email addresses receive attributions in the registry with public profile tracking. Authors without email are displayed by name only.

#### File-Based Fields

The `description`, `license-text`, and `readme` fields support file-based content using the `{ path = "..." }` syntax:
...
# File-based fields - content read from files
...
- Paths are resolved relative to the `schema.toml` location
- Both relative (`./README.md`) and absolute paths are supported
- Files MUST be UTF-8 encoded

**Publish Behaviour:**

When publishing to a registry, file-based fields are **read and inlined** at publish time:

1. The file is read from disk
2. Content is validated (UTF-8, size limits)
3. The inline content is stored in the registry

This ensures registry packages are self-contained and don't depend on external files.

**Size Limits:**

Size limits for file-based fields are enforced at publish time. See the registry specification for current limits.
...
> [!NOTE]
> Both inline strings and file-based syntax are valid. Choose based on content length and maintainability preferences.
...
```toml title="schema.toml" collapse={8-14}
version = "v1"
...
| `package.description`     | String or `{ path: String }` | No       |
...
| `package.license`         | String (SPDX identifier)     | No       |
| `package.license-text`    | String or `{ path: String }` | No       |
| `package.readme`          | String or `{ path: String }` | No       |
| `dependencies.*`          | Dependency spec              | No       |
| `files.exclude`           | Array of String (glob)       | No       |

#### String or Path Union

Fields supporting file-based content accept either:
...
See [SPEC-0018](/specs/spec/SPEC-0018) for implementation details.
...
- [ ] AC-7: File-based fields (`{ path = "..." }`) are read and validated
- [ ] AC-8: File-based fields are inlined during `kintsu publish`
- [ ] AC-9: Clear errors for missing or unreadable files (KPK4003, KPK4004)
