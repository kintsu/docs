...
```text title="Interactive token entry"
$ kintsu login
Token: ******
...
```bash title="Direct token authentication"
# Direct token
...
```text title="Plaintext credential warning"
...
```bash title="Suppress confirmation"
...
```text title="kintsu login (interactive)"
$ kintsu login
Token: ******
✓ Logged in to registry.kintsu.dev (kintsu-public) successfully
...
```text title="kintsu login --no-keychain"
$ kintsu login --no-keychain
Token: ******
✓ Logged in to registry.kintsu.dev (kintsu-public) successfully
...
```text title="kintsu logout output"
...
```text title="kintsu whoami"
  registry.kintsu.dev: josh@kintsu.dev
```

**All registries:**

```text title="kintsu whoami --all"
  registry.kintsu.dev: josh@kintsu.dev
