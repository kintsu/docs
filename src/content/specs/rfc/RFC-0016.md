...
- Only `struct` operands are allowed. If any operand is not a struct (enum, error, primitive, alias resolving to non-struct), the compiler emits an error (same behaviour as RFC-0007 for invalid union operands).
...
- `&|` is left-associative. Expressions are evaluated left-to-right. Parentheses are honoured and inner parenthesized expressions are evaluated first.
...
`&|` is additive; it introduces new syntax and semantics but does not change existing behaviour. Existing code remains valid. Any future changes to codegen defaults or representation will be opt-in and documented with migration guidance.

## References

- [RFC-0007](/specs/rfc/RFC-0007) - Union Type Support (field-merging `&` semantics)
