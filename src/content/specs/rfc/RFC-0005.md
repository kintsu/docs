...
This section summarizes the syntax and association behaviour. Detailed normative rules appear in [TSY-0005](/specs/tsy/tsy-0005) and [SPEC-0005](/specs/spec/spec-0005).
...
- [ ] AC-3: The RFC clarifies the anonymous struct extraction behaviour for struct variants (ErrorName + VariantName -> generated struct name).
...
This RFC documents the existing error type behaviour. Future changes that alter the variant extraction naming algorithm (e.g., changing case conventions or adding disambiguation suffixes) would be breaking for tooling. Such changes require explicit migration guidance.
