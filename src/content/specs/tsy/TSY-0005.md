...
Error types provide structured, type-safe representations of failure cases for operations. This TSY documents the exact syntax and variant semantics so schema authors and code generators can rely on consistent error behaviour. The anonymous struct extraction algorithm for struct variants is normative — implementations must generate the same struct names to ensure interoperability.
