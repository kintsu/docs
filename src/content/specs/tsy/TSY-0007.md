...
Union types provide field composition through merging. This TSY documents the exact merging semantics and conflict resolution rules so schema authors and code generators can rely on consistent behaviour. The left-to-right precedence rule for field conflicts is normative — implementations must resolve conflicts deterministically to ensure interoperability.
...
> [!NOTE]
> Anonymous struct extraction ([TSY-0003](/specs/tsy/tsy-0003)) occurs before union resolution, so anonymous operands are treated as named structs during merging.
