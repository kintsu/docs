...
updates:
  - author: joshua-auchincloss
    date: 2025-12-31
    description: Updated Rule 11 to include all form-required types (decimal, duration, f8)
  - author: joshua-auchincloss
    date: 2025-12-28
    description: Added form metadata (Rule 11) and error suppression (Rule 12)
  - author: joshua-auchincloss
    date: 2025-10-30
...
This specification defines the normative type system rules for metadata annotations in Kintsu schemas. Metadata uses attribute syntax to annotate schema elements with version information, error type defaults, serialization formats, and error suppression. The system distinguishes between **outer attributes** (`#[...]`) that apply to specific items and **inner attributes** (`#![...]`) that apply to containing scopes and their children.
...
- **Form metadata**: Serialization format specification with `#[form(...)]`
- **Error suppression**: Warning acknowledgement with `#[allow(error_name)]`
...
5. **Extensibility**: Attribute syntax supports metadata types including version, error, form, and allow
...
### Rule 11: Form metadata syntax

Form metadata specifies canonical serialization formats for fields and namespaces.

**Syntax**: `form(format, ...)` where format is a format identifier or chain
...
#[form(base64 = gzip = json)] data: binary
#[form(precision = 10, scale = 2)] price: decimal
#[form(ms)] elapsed: duration
#[form(e4m3)] weight: f8
...
1. Inner form attributes provide namespace defaults for child fields
2. Outer form attributes override namespace defaults for specific fields
3. Form chaining uses `=` operator for right-to-left composition
4. Form-required types (`complex`, `decimal`, `duration`, `f8`) MUST have explicit form attributes
...
namespace api {
    ...
    struct Signal {
        angle: complex,           // Inherits polar form
        #[form(cartesian, precision = f32)] pos: complex  // Override
    }
}
```

> [!IMPORTANT]
> Form metadata is validated against field types. Invalid format-type combinations produce compilation errors. Form-required types (`complex`, `decimal`, `duration`, `f8`) without form attributes are compilation errors. See [TSY-0015](/specs/tsy/tsy-0015) for detailed format rules.

### Rule 12: Error suppression syntax

Allow metadata suppresses specific warnings by error name.

**Syntax**: `allow(error_name)` where error_name is a snake_case identifier

**Valid forms:**

```kintsu
// Field-level suppression (outer attribute)
#[allow(global_compositional_default)]
#[form(base64 = json)] data: binary

// Namespace-level suppression (inner attribute)
namespace api {
    #![allow(global_compositional_default)]
    #![form(base64 = gzip = json)]
    // All fields inherit suppression
}
```

**Scope rules:**

1. Inner allow attributes suppress warnings for all items in namespace
2. Outer allow attributes suppress warnings for specific items only
3. Allow attributes do not propagate to nested namespaces
4. Only category 8 (Warning) errors are suppressible by default
...
1. Error code category changes from `8` to `7`
2. Severity changes from Warning to Info
3. Diagnostic hidden from default CLI output
4. Diagnostic shown with `--verbose` flag

> [!WARNING]
> Only suppressible errors can be allowed. Attempting to suppress non-suppressible errors (categories 0-6, 9) produces a compilation error. See [ERR-0001](/specs/err/err-0001) for category definitions.
